...
\defn{Permutation (Anordnung, eigentlich Fakultät)}{
...
\begin{equation}
    ...
    \end{split}
\end{equation}
Mit dem Ti-Nspire CX II-T: \textbf{nCr(n,k)}
}
...
\begin{equation}
    P(\Omega)=1
\end{equation}
\textbf{Vereinigung}. Sind die Ereignisse \(A_1,A_2,\dots\) \textbf{paarweise disjunkt}, also \(A_i \cap A_j = \emptyset\) für \(i \neq j\), dann gilt
\begin{equation}
...
\subsection{Warum braucht man den Satz von Bayes, um einen Spamfilter zu bauen?}
Ein Spamfilter muss die Wahrscheinlichkeit berechnen, dass eine E-Mail Spam ist,
gegeben bestimmte Wörter oder Merkmale. 
Der Satz von Bayes erlaubt, diese bedingte Wahrscheinlichkeit zu berechnen:
\begin{equation}
    P(\text{Spam}|\text{Merkmale}) = P(\text{Merkmale}|\text{Spam}) \frac{P(\text{Spam})}{P(\text{Merkmale})}
\end{equation}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Images/erwartungswert-varianz.png}
...
Das Gesetz der grossen Zahlen besagt, dass sich die relative Häufigkeit eines
Ereignisses bei zunehmender Anzahl von Experimenten der theoretischen Wahrscheinlichkeit
dieses Ereignisses annähert.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.35\linewidth]{Images/lineare-regression.png}
...
Eine Verteilungsfunktion \(F\) kann immer zerlegt werden in eine stückweise konstante,
 monoton wachsende, rechtsseitig stetige Funktion \(F_d\) und eine stetige Funktion \(F_s\).
}

\defn{Diskrete Wahrscheinlichkeitsverteilung}{
    \begin{equation}
        p(x):
        \begin{cases}
            S \to &\mathbb{R} \\
        ...
        \end{cases}
    \end{equation}
    Die Funktion \(p(x)\) heisst diskrete Wahrscheinlichkeitsverteilung auf der Menge \(S\).
    Für \(p(x)\) gilt:
    \begin{equation}
        \begin{split}
            P(A) &= \sum_{x \in A \cap S} p(x) \\
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Images/übersicht-verteilung.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Images/spannweitequartilabstand.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Images/verteilungsfunktion-erwartungswert.png}
...
Wahrscheinlichkeitsdichte:
    ...
    \end{equation}
    Die Funktion \(\varphi(x)\) heisst Wahrscheinlichkeitsdichte der Verteilungsfunktion \(F\) falls gilt:
    \begin{equation}
        P(X \leq x) = F(x) = \int_{-\infty}^{x} \varphi(\xi) d\xi
    \end{equation}
    Man spricht in diesem Fall von einer \textbf{stetigen Wahrscheinlichkeitsverteilung}.
}
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Images/integrationsregeln.png}
    \caption{Integrationsregeln}
\end{figure}
...
\defn{Gleichverteilung }{
    \begin{equation}
        X \sim U(a,b) \text{ or } X \sim U(a,b)
    \end{equation}
    \begin{equation}
        \varphi(x)=
        \begin{cases}
...
\defn{Gedächtnisloser Prozess}{
    ...
    einem Intervall eintritt, immer gleich ist,
...
\exm{Herleitung}{
    \begin{equation*}
        \begin{split}
            G(X) \quad &= 1-F(x) \\
            \frac{P(T > x + y)}{P(T>x)} \quad &= P(T > y) \\
            \frac{G(X+y)}{G(X)} \quad &= G(y)
        \end{split}
    \end{equation*}

    Daraus folgt Ableitung nach \(y\), bei \(y=0\):
    \begin{equation*}
        \begin{split}
            \Rightarrow \quad &G'(x) = G'(0)G(X)\\
            \Rightarrow \quad &a = -G'(0) \\
            \Rightarrow \quad &G(x) = Ce^{-ax} \\
            \Rightarrow \quad &\varphi(x) = aCe^{-ax} \\
            \Rightarrow \quad &\text{für } x>0\text{, und Normierung: }C=1
        \end{split}
    \end{equation*}
}
...
\defn{Erlang-Verteilung }{
    \begin{equation}
        X \sim Erlang_k(a)
    \end{equation}
    \begin{equation}
        \varphi_k(x)= 1-e^{-ax} \sum_{i=0}^{k-1} \frac{(ax)^i}{i!}, x \geq 0
    \end{equation}
    Für \(k =\) Anzahl von Summanden.

    \begin{equation}
        F_k(x) = a^k \frac{k^{k-1}}{(k-1)!} e{-ax}, x \geq 0
    \end{equation}
...
\defn{Normalverteilung}{
    ...
    \end{equation}
    \begin{equation}
        \varphi(x) = \frac{1}{\sqrt{2\pi\sigma}} e^{-\frac{x-\mu}{2\sigma^2}}
    \end{equation}
}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\linewidth]{Images/fehlerabschätzung-norm.png}
...
Man beachte, dass die Zufallsvariable Z nicht mehr normalverteilt ist, wenn man \(\mu\) und \(\sigma\)
durch \textbf{Schätzwerte} ersetzt, die resultierende Verteilung ist dann eine \textbf{t-Verteilung}.
...
\defn{Potenzverteilung}{
    ...
    \begin{equation}
        \begin{split}
            \varphi(x) &= \frac{\alpha - 1}{x_{min}}(\frac{x}{x_{min}})^{-\alpha}, \text{für } x > x_{min}\\
            F(x) &= 1- (\frac{x}{x_{min}})^{1- \alpha} \text{für } x > x_{min}
        \end{split}
    \end{equation}
}
...
\defn{Kennzahlen}{
    \begin{equation}
        ...
        \end{split}
    \end{equation}
    Man beachte die Werte für die die Funktion nicht definiert ist!
...
\defn{Binomial Verteilung }{
    ...
    \begin{equation}
        \begin{split}
            ...
            F(x) &= \sum_{i=0}^{k} \binom{n}{i} p^i(1-p)^{n-i}
        \end{split}
    \end{equation}
}
...
\end{equation}
Für eine \text{genauere Approximation}, kann folgendes benutzt werden:
\begin{equation}
    P(a < X \leq b) = \phi(\frac{b+0.5-np}{\sqrt{np(1-p)}}) - \phi(\frac{a+0.5-np}{\sqrt{np(1-p)}})
\end{equation}

\defn{Faustregel}{
    Man benutzt die \textbf{Normalaproximation} wenn:
    \begin{equation}
        np \geq 5 \land n(1-p) \geq 5
    \end{equation}
    Unter diesen Bedingungen nähert sich die Binomialverteilung gut einer Normalverteilung an.
    Wir benutzen die \textbf{Poisson-Verteilung} wenn der Erwartungswert klein ist und
    die Anzahl der Versuche n sehr gross ist, aber mit geringen Wahrscheinlichkeiten p.

    \begin{equation}
        n >50 \land p < 0.1 \land np=\lambda \leq 10
    \end{equation}
    Also wenn p klein und n gross ist. Dies ist also besonders bei seltenen Ergeignisse nützlich.
}
...
\defn{Poisson-Verteilung}{
    ...
    Für das Intervall \([0,x]\) gilt:
    \begin{equation}
        \lambda = ax  \text{ (e.g aus exponentieller ZV)}
    \end{equation}
    Dann gilt Wahrscheinlichkeit für \(k\)-Ereignisse im Intervall \([0,x]\),
    wenn die Zeit zwischen den Ereignissen exponentialverteilt ist mit Dichte Exp(a).
    (Anzahl Ereignisse mit exponentialverteilten Intervall)
    Für die Approximation der Binomialverteilung gilt:
    \begin{equation}
        \lambda = np
    \end{equation}
}
...
\defn{Probabilistischer Beweis}{
    Ein probabilistischer Beweiss kann keine absoluten Sicherheiten liefern,
    sondern nur Aussagen über Wahscheinlichkeiten ermöglichen.
}
...
\defn{t-Test}{
    ...
    \(H_0\): \(x_1, \dots ,x_n\) und \(y_1, \dots , y_n\) mit gleicher Varianz und Erwartungswert.

    T-Test funktioniert nur, wenn sich die Mittelwerte berechnen lassen!
    Bei randomisierten Experimenten nicht möglich.
}
...
\defn{Diskrepanz (Für diskrete Verteilungstests, \(\chi^2\)-Tests)}{
    Die Diskrepanz ist eine Teststatistik für diskrete Zufallsvariabeln.
    Die Teststatistik ist symmetrisch.
    \begin{equation}
        D = \sum_{i=1}^{n} \frac{(n_i - np_i)^2}{np_i}
    \end{equation}
    Die Diskrepanz \(D\) ist für genügend grosse \(n\) annähernd \(\chi^2\)-verteilt mit \(d\)
    Freiheitsgraden.
    "Genügend gross" heisst nach einer verbreiteten Faustregel: so gross, dass \(n_i > 5 \forall i\).
}
...
\defn{F-Test}{
    ...
    Sind \(X\) und \(Y\) \(\chi^2\)-verteilte Zufallsvariable mit \(n\) bzw. \(m\) Freiheitsgraden, dann
    heisst die Verteilung von \(\frac{X}{Y}\) eine F-Verteilung mit 
    n,m-Freiheitsgraden, auch \(F_{n,m}\) geschrieben.

    Mit dem F-Test können wir verschiedene Erwartungswerte normalverteiler ZV vergleichen.
    Wir haben \(X_i\) mit \(var(X_i)= \sigma^2\) und wollen herausfinden ob sich \(\mu_i = E(X_i)\) unterscheiden.
    Dabei ist die Nullhypothese:
    \begin{equation*}
        \mu_i = \mu_j \forall i,j
    \end{equation*}

    Wir verwenden für die Test-Statistik:
    \begin{equation}
        \begin{split}
            F &= \frac{\text{Varianz über alle }i}{\text{Varianz für die Gruppe }i} \\
            F &= \frac{S_X^2}{S_Y^2}
        \end{split}
    \end{equation}
    \(S_X\) ist die geschätzte Varianz von \(X\) mit \(n-1\) Freiheitsgraden und ist
    durch das Quadrieren \(\chi^2\)-Verteilt. Bei \(S_Y\) dasselbe.

    \begin{enumerate}
        \item Messungen für alle \(i\) mehrfach messen: \(x_{ij}\)
        \item Berechne die empirische Varianz für alle Gruppen und die Gruppe \(i\)
        \item Die Teststatistik ausrechnen
        \item Nullhypothese: \(F=1\) resp. \(H_0\): \(\mu_i = \mu_j \forall i,j\) resp.
        Experiment \(X\) hat Varianz \(\sigma^2\)
        \item Verteilung von F: Fisher-Verteilt resp. F-Verteilt
    \end{enumerate}
}
...
\defn{\(\chi^2\) Verteilung}{
    Sind \(X_i\) unabhängige standardnormalverteilte ZV, dann ist:
    \begin{equation*}
        \chi_n^2 = \sum_{i=1}^{n} X_i^2
    \end{equation*}
    eine \(\chi^2\) verteilte Zufallsvariabel mit \(n\) (resp. \(k\)) Freiheitsgraden.
...
\defn{\(\chi^2\)-Test}{
    Wir wollen den Umstand testen, dass die Anzahlen \(X_i\) zu den Wahrscheinlichkeiten \(p_i\) der Ausgänge \(i\) gehören.
    Dafür nehmen wir die Diskrepanz der quadrierten ZV als Teststatistik:
    \begin{equation*}
        D_k = X_1^2 + \dots + X_k^2
    \end{equation*}
    Dabei sind die standardnormalen \(X_i\), \(\chi^2\)-verteilt mit k-1 Freiheitsgraden.
    Nun können wir prüfen, ob:
    \begin{equation*}
        D > D_+ \Rightarrow \text{Daten passen nicht (H verwerfen)}
    \end{equation*}
    Und:
    \begin{equation*}
        D < D_- \Rightarrow \text{Daten passen zu gut}
    \end{equation*}
    Für die Quantilen nehmen wir:
    \begin{equation*}
        \begin{split}
            F_D(D_+) &= P(D \leq D_+) = 1-\alpha \\
            F_D(D_-) &= P(D \leq D_-) = \alpha
        \end{split}
    \end{equation*}
...
\defn{K-Test}{
    Der K-Test wird benutzt, um zu testen, ob eine Zufallsvariable einer bestimmten Verteilung folgt.
    Dafür müssen Ergebnisse in einer empirischen Verteilfunktion \(F_n(x)\) und einer theoretischen Verteilfunktion \(F(x)\) verglichen werden.
    Der Test basiert auf der maximalen Differenz zwischen diesen beiden Funktionen:
    \begin{equation*}
        D_n = \sup_x |F_n(x) - F(x)|
    \end{equation*}
    Der kritische Wert \(D_{n,\alpha}\) wird aus der Kolmogorov-Smirnov-Tabelle entnommen, abhängig von der Stichprobengröße \(n\) und dem Signifikanzniveau \(\alpha\).
    Wenn \(D_n > D_{n,\alpha}\), wird die Nullhypothese, dass die Zufallsvariable der theoretischen Verteilung folgt, verworfen.

    \begin{equation*}
        \begin{split}
            K_n^+ &= \sqrt{n} \text{max}_x(F_{\text{empirisch}} - F(X)) = \sqrt{n} \text{max}_j(\frac{j}{n}-F(x_j))\\
            K_n^- &= \sqrt{n} \text{max}_x(F(X) - F_{\text{empirisch}}) = \sqrt{n} \text{max}_j(F(x_j) - \frac{j-1}{n})
        \end{split}
    \end{equation*}
    Der Faktor \(\sqrt{n}\)  wird verwendet um einfache zu rechnen.

    \(H_0\): Die ZV X hat die Verteilung \(F(X)\).
}

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{Images/k-tabelle.png}
    \caption{K-Tabelle}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{Images/kolmogorov-tabelle.png}
