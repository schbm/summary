...
\begin{equation}
    \begin{split}
        \hat{a} &= \frac{n+1}{n} min(x_n) \\
        \hat{b} &= \frac{n+1}{n} max(x_n)
    \end{split}
\end{equation}
...
\defn{Exponential Verteilung }{
    \begin{equation}
        \varphi(x)= 1-e^{-ax} \sum_{i=0}^{k-1} \frac{(ax)^i}{i!}, x \geq 0
    \end{equation}
    Für \(k =\) Anzahl von Summanden.

    \begin{equation}
        F(x) = a^k \frac{k^{k-1}}{(k-1)!} e{-ax}, x \geq 0
    \end{equation}
...
\defn{Binomial Verteilung }{
    ...
    \begin{equation}
        \begin{split}
            \text{Wahrscheinlichkeit} \quad P(X=k) &= \binom{n}{k} p^k (1-p)^{n-k} \\
            \text{Verteilung} \quad F(x) &= \sum_{i=0}^{k} \binom{n}{i} p^i(1-p)^{n-1}
        \end{split}
    \end{equation}
}
...
\defn{\(\chi^2\) Verteilung}{
    ...
    \end{equation*}
    eine \(\chi^2\) verteilte Zufallsvariabel mit n Freiheitsgraden.
